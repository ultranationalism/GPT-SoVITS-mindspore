...
class StochasticDurationPredictor(nn.Cell):
    def __init__(
        self,
        in_channels,
        filter_channels,
...
class TextEncoder(nn.Cell):
    ...
    def construct(self, y, text, ge):
        ...
        text = self.text_embedding(text).swapaxes(1, 2)
...
class ResidualCouplingBlock(nn.Cell):
    def __init__(
        self,
        channels,
        hidden_channels,
...
class PosteriorEncoder(nn.Cell):
    def __init__(
        self,
        in_channels,
        out_channels,
...
class MultiPeriodDiscriminator(torch.nn.Cell):
    def __init__(self, use_spectral_norm=False):
        ...
        self.discriminators = nn.CellList(discs)
...
class ReferenceEncoder(nn.Cell):
    """
    ...
    def __init__(self, spec_channels, gin_channels=0):
        ...
        # self.wns = nn.CellList([weight_norm(num_features=ref_enc_filters[i]) for i in range(K)])
...
class Quantizer(torch.nn.Cell):
    def __init__(self, embed_dim=512, n_code_groups=4, n_codes=160):
        ...
        self.quantizer_modules = nn.CellList(
            [
    ...
    def construct(self, xin):
        # B, C, T
        B, C, T = xin.shape
        xin = xin.swapaxes(1, 2)
        x = xin.reshape(-1, self.embed_dim)
        x = ops.split(x, self.embed_dim // self.n_code_groups, axis=-1)
        min_indicies = []
        z_q = []
    ...
    def embed(self, x):
        ...
        for q, embed in zip(x, self.quantizer_modules):
            ...
            ret.append(q)
        ret = ops.cat(ret, -1)
        return ret.swapaxes(1, 2)  # N, C, T


class CodePredictor(nn.Cell):
    def __init__(
        self,
        hidden_channels,
        filter_channels,
    ...
    def construct(self, x, x_mask, refer, codes, infer=False):
        x = x.detach()
        ...
        logits = x.reshape(x.shape[0], self.n_q - 1, self.dims, x.shape[-1]).swapaxes(
            2, 3
        )
...
class SynthesizerTrn(nn.Cell):
    ...
    def construct(self, codes, text, refer):
