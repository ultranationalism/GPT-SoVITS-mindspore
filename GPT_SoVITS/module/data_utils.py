...
from mindnlp.transformers import HubertModel
...
class TextAudioSpeakerLoader():
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        text = ms.Tensor(phoneme_ids)
        try:
            spec, wav = self.get_audio("%s/%s" % (self.path5, audiopath))
            ssl = ms.load_checkpoint("%s/%s.ckpt" % (self.path4, audiopath))
            if (ssl.shape[-1] != spec.shape[-1]):
                typee = ssl.dtype
                ssl = ops.pad(ssl.float(), (0, 1), mode="replicate").to(typee)
            ssl.requires_grads = False
        except:
            traceback.print_exc()
            spec = ops.zeros((1025, 100))
            wav = ops.zeros((1, 100 * self.hop_length))
            ssl = ops.zeros((1, 768, 100))
            text = text[-1:]
            print("load audio or ssl error!!!!!!", audiopath)
        return (ops.stop_gradient(ssl), spec, wav, text)

    def get_audio(self, filename):
        audio_array = load_audio(filename, self.sampling_rate)  # load_audio的方法是已经归一化到-1~1之间的，不用再/32768
        audio = ms.Tensor(audio_array)  # /32768
        audio_norm = audio
        audio_norm = audio_norm.unsqueeze(0)
        spec = spectrogram_torch(audio_norm, self.filter_length, self.sampling_rate, self.hop_length, self.win_length,
                                  center=False)
        spec = ops.squeeze(spec, 0)
        return spec, audio_norm

    def get_sid(self, sid):
        sid = ms.Tensor([int(sid)])
        return sid
...
class DistributedBucketSampler(ms.dataset.DistributedSampler):
    """
    ...
    def __init__(self, batch_size, boundaries, num_replicas=None, rank=None, shuffle=True):
        super().__init__(num_shards=num_replicas, shard_id=rank, shuffle=shuffle)
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            for bucket in self.buckets:
                indices.append(ops.randperm(len(bucket)).tolist())
        else:
            for bucket in self.buckets:
        ...
        if self.shuffle:
            batch_ids = ops.randperm(len(batches)).tolist()
